#!/usr/bin/env python3
"""Benchmark latinum vs normal Claude output token counts."""
...
SKILL_PATH = REPO_DIR / "skills" / "latinum" / "SKILL.md"
...
def run_benchmarks(client, model, prompts, latinum_system, trials):
    results = []
    total = len(prompts)
    ...
    for i, prompt_entry in enumerate(prompts, 1):
        ...
        for mode, system in [("normal", NORMAL_SYSTEM), ("latinum", latinum_system)]:
...
def compute_stats(results):
    ...
    for entry in results:
        ...
        latinum_medians = statistics.median(
            [t["output_tokens"] for t in entry["latinum"]]
        )
        savings = 1 - (latinum_medians / normal_medians) if normal_medians > 0 else 0
        ...
        rows.append(
            {
                ...
                "latinum_median": int(latinum_medians),
    ...
    avg_latinum = round(statistics.mean([r["latinum_median"] for r in rows]))
...
def format_table(rows, summary):
    lines = [
        "| Task | Normal (tokens) | Latinum (tokens) | Saved |",
        "|------|---------------:|----------------:|------:|",
    ]
    for r in rows:
        label = format_prompt_label(r["id"])
        lines.append(
            f"| {label} | {r['normal_median']} | {r['latinum_median']} | {r['savings_pct']}% |"
        )
    lines.append(
        f"| **Average** | **{summary['avg_normal']}** | **{summary['avg_latinum']}** | **{summary['avg_savings']}%** |"
    )
    lines.append("")
    lines.append(
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark latinum vs normal Claude")
    ...
    latinum_system = load_latinum_system()
    ...
    results = run_benchmarks(client, args.model, prompts, latinum_system, args.trials)
