#!/usr/bin/env node
// Tests for src/tools/latinum-init.js — fixture-based.
// Run: node tests/test_latinum_init.js
...
const INIT = path.join(ROOT, 'src', 'tools', 'latinum-init.js');
...
function test(name, fn) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'latinum-init-test-'));
  try {
    fn(tmp);
    passed++;
    console.log(`  ✓ ${name}`);
...
console.log('latinum-init tests\n');

test('greenfield: creates all rule files with proper frontmatter', (tmp) => {
  runInit(tmp);
  const cursor = fs.readFileSync(path.join(tmp, '.cursor/rules/latinum.mdc'), 'utf8');
  assert.match(cursor, /alwaysApply: true/);
  assert.match(cursor, /Respond terse like smart latinum/);
  const windsurf = fs.readFileSync(path.join(tmp, '.windsurf/rules/latinum.md'), 'utf8');
  assert.match(windsurf, /trigger: always_on/);
  const cline = fs.readFileSync(path.join(tmp, '.clinerules/latinum.md'), 'utf8');
...
test('append mode: existing AGENTS.md gets latinum appended (not replaced)', (tmp) => {
  fs.writeFileSync(path.join(tmp, 'AGENTS.md'), '# My project\n\nDo not delete me.\n');
  runInit(tmp);
  const agents = fs.readFileSync(path.join(tmp, 'AGENTS.md'), 'utf8');
  assert.match(agents, /Do not delete me/);
  assert.match(agents, /Respond terse like smart latinum/);
});

test('skip mode: existing .cursor rule is not overwritten without --force', (tmp) => {
  const dir = path.join(tmp, '.cursor/rules');
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, 'latinum.mdc'), '# original\nDo not delete me.\n');
  const out = runInit(tmp);
  assert.match(out, /\? .*\.cursor\/rules\/latinum\.mdc/);
  const after = fs.readFileSync(path.join(dir, 'latinum.mdc'), 'utf8');
...
test('--only filters to one target', (tmp) => {
  ...
  assert.ok(fs.existsSync(path.join(tmp, '.clinerules/latinum.md')));
  assert.ok(!fs.existsSync(path.join(tmp, '.cursor')));
});

test('detects sentinel and skips files that already have latinum content', (tmp) => {
  // Hand-write a file that already contains the rule (simulating prior install).
  const dir = path.join(tmp, '.clinerules');
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, 'latinum.md'),
    '# Existing\n\nRespond terse like smart latinum. Hello.\n');
  const out = runInit(tmp, '--only', 'cline');
