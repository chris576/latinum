...
function freshTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'latinum-freshinstall-'));
}
...
function runInstaller(args, configDir, extraEnv = {}) {
  return spawnSync('node', [INSTALLER, ...args, '--config-dir', configDir, '--non-interactive', '--no-mcp-inflate'], {
...
function latinumHookCommands(settings, event, marker) {
...
test('fresh install populates hooks dir and settings.json (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH; the claude provider is the only path that wires hooks' }, () => {
  ...
  try {
    ...
    assert.ok(fs.existsSync(path.join(hooks, 'latinum-activate.js')),     'latinum-activate.js missing');
    assert.ok(fs.existsSync(path.join(hooks, 'latinum-mode-tracker.js')), 'latinum-mode-tracker.js missing');
    assert.ok(fs.existsSync(path.join(hooks, 'latinum-config.js')),       'latinum-config.js missing');
    ...
    assert.ok(SETTINGS.hasLatinumHook(settings, 'SessionStart', 'latinum-activate'),
      'SessionStart hook missing or wrong marker');
    assert.ok(SETTINGS.hasLatinumHook(settings, 'UserPromptSubmit', 'latinum-mode-tracker'),
      'UserPromptSubmit hook missing or wrong marker');
    assert.ok(settings.statusLine, 'statusLine not set');
    assert.match(getStatuslineCommand(settings), /latinum-statusline/,
      'statusLine command does not reference latinum');
  } finally {
...
test('idempotent install does not duplicate hook entries (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH' }, () => {
  ...
  try {
    ...
    const sessStart = latinumHookCommands(settings, 'SessionStart', 'latinum-activate');
    assert.equal(sessStart.length, 1, `expected 1 SessionStart latinum hook, got ${sessStart.length}`);

    const ups = latinumHookCommands(settings, 'UserPromptSubmit', 'latinum-mode-tracker');
    assert.equal(ups.length, 1, `expected 1 UserPromptSubmit latinum hook, got ${ups.length}`);
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('uninstall strips latinum hooks but preserves user-authored ones (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH; uninstall test depends on a prior real install' }, () => {
  const dir = freshTmpDir();
  try {
    ...
    if (fs.existsSync(hooks)) {
      for (const f of ['latinum-activate.js', 'latinum-mode-tracker.js', 'latinum-config.js', STATUSLINE_FILE]) {
    ...
    // Settings cleaned up.
    const settings = JSON.parse(fs.readFileSync(path.join(dir, 'settings.json'), 'utf8'));
    // No remaining latinum-marked hooks anywhere.
    for (const ev of Object.keys(settings.hooks || {})) {
      const arr = settings.hooks[ev] || [];
      for (const e of arr) {
        for (const h of (e.hooks || [])) {
          assert.doesNotMatch(h.command || '', /latinum/, `${ev} still has latinum hook: ${h.command}`);
        }
      }
    }
    // User's pre-existing hook preserved.
    const preservedUser = latinumHookCommands(settings, 'SessionStart', 'user-owned-hook').length > 0;
    assert.ok(preservedUser, 'user-authored SessionStart hook was wiped during uninstall');

    // Statusline pointing at latinum should be removed.
    assert.doesNotMatch(getStatuslineCommand(settings), /latinum-statusline/,
      'latinum statusline survived uninstall');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
`);
    ...
    // Latinum hooks must be wired.
    assert.ok(SETTINGS.hasLatinumHook(parsed, 'SessionStart', 'latinum-activate'));
    assert.ok(SETTINGS.hasLatinumHook(parsed, 'UserPromptSubmit', 'latinum-mode-tracker'));
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
const SKILL_BODY_SRC = path.join(REPO_ROOT, 'skills', 'latinum', 'SKILL.md');
...
test('openclaw install writes skill folder + SOUL.md bootstrap', () => {
  ...
  fs.mkdirSync(ws);
  try {
    const r = spawnSync('node', [INSTALLER, '--only', 'openclaw', '--non-interactive', '--no-mcp-inflate', '--config-dir', dir], {
    ...
    const skillFile = path.join(ws, 'skills', 'latinum', 'SKILL.md');
    ...
    assert.match(soulRaw, /<!-- latinum-begin -->/, 'SOUL.md missing begin marker');
    assert.match(soulRaw, /<!-- latinum-end -->/, 'SOUL.md missing end marker');
    assert.match(soulRaw, /Respond terse like smart latinum/, 'SOUL.md missing sentinel');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('openclaw install is idempotent: skill frontmatter not double-prepended, SOUL.md has one marker block', () => {
  ...
  try {
    ...
    const args = ['--only', 'openclaw', '--non-interactive', '--no-mcp-inflate', '--config-dir', dir];
    spawnSync('node', [INSTALLER, ...args], { env, encoding: 'utf8' });
    spawnSync('node', [INSTALLER, ...args], { env, encoding: 'utf8' });

    const skillRaw = fs.readFileSync(path.join(ws, 'skills', 'latinum', 'SKILL.md'), 'utf8');
    ...
    const beginMatches = soulRaw.match(/<!-- latinum-begin -->/g) || [];
...
test('openclaw install preserves user content in SOUL.md (append, not overwrite)', () => {
  ...
  try {
    spawnSync('node', [INSTALLER, '--only', 'openclaw', '--non-interactive', '--no-mcp-inflate', '--config-dir', dir], {
      env: { ...process.env, OPENCLAW_WORKSPACE: ws, NO_COLOR: '1' },
      encoding: 'utf8',
    });
    const soulRaw = fs.readFileSync(path.join(ws, 'SOUL.md'), 'utf8');
    ...
    assert.match(soulRaw, /<!-- latinum-begin -->/, 'latinum block not appended');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('openclaw uninstall removes skill folder + strips SOUL.md block, preserving user content', () => {
  ...
  try {
    const env = { ...process.env, OPENCLAW_WORKSPACE: ws, NO_COLOR: '1' };
    spawnSync('node', [INSTALLER, '--only', 'openclaw', '--non-interactive', '--no-mcp-inflate', '--config-dir', dir], { env, encoding: 'utf8' });

    // Strip claude/gemini from PATH so uninstall doesn't touch real plugins.
    const cleanPath = pathWithout(['claude', 'gemini']);
    const r = spawnSync('node', [INSTALLER, '--uninstall', '--non-interactive', '--no-mcp-inflate', '--config-dir', dir], {
      env: { ...env, PATH: cleanPath },
      encoding: 'utf8',
    });
    assert.notEqual(r.status, 2, `uninstall argv error: ${r.stderr}`);

    assert.equal(fs.existsSync(path.join(ws, 'skills', 'latinum')), false, 'skill folder should be removed');
    const soulAfter = fs.readFileSync(path.join(ws, 'SOUL.md'), 'utf8');
    assert.doesNotMatch(soulAfter, /<!-- latinum-begin -->/, 'latinum block survived uninstall');
    assert.doesNotMatch(soulAfter, /<!-- latinum-end -->/, 'latinum end marker survived uninstall');
...
test('latinum-init.js --only openclaw routes through the same helper', () => {
  const dir = freshTmpDir();
  const ws = path.join(dir, 'ws');
  fs.mkdirSync(ws);
  try {
    const initScript = path.join(REPO_ROOT, 'src', 'tools', 'latinum-init.js');
    const r = spawnSync('node', [initScript, dir, '--only', 'openclaw'], {
      env: { ...process.env, OPENCLAW_WORKSPACE: ws, NO_COLOR: '1' },
      encoding: 'utf8',
    });
    assert.equal(r.status, 0, `latinum-init failed: ${r.stderr || r.stdout}`);
    assert.ok(fs.existsSync(path.join(ws, 'skills', 'latinum', 'SKILL.md')), 'skill missing via init route');
    assert.ok(fs.existsSync(path.join(ws, 'SOUL.md')), 'SOUL.md missing via init route');
    const soulRaw = fs.readFileSync(path.join(ws, 'SOUL.md'), 'utf8');
    assert.match(soulRaw, /Respond terse like smart latinum/, 'sentinel missing via init route');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('lib settings.addCommandHook is idempotent across two synthetic install passes', () => {
  ...
  try {
    ...
    SETTINGS.addCommandHook(settings, 'SessionStart', {
      command: '"/usr/bin/node" "/abs/hooks/latinum-activate.js"',
      marker: 'latinum-activate',
    });
    SETTINGS.addCommandHook(settings, 'SessionStart', {
      command: '"/usr/bin/node" "/different/hooks/latinum-activate.js"',
      marker: 'latinum-activate',
    });
