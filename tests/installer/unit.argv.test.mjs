...
test('--list prints provider matrix', () => {
  const r = run('--list');
  assert.equal(r.status, 0);
  assert.match(r.stdout, /latinum provider matrix/);
...
test('--only with unknown agent id exits 2', () => {
  ...
  assert.match(r.stderr, /latinum --list/);
});
...
test('--config-dir expands ~ to home directory', async () => {
  ...
  // match even when the latinum plugin is already installed.
...
test('bare --with-mcp-inflate (no upstream) exits 2 with hint', () => {
  // Regression for issue where --with-mcp-inflate registered a stub MCP entry
  // that crashed on every Claude Code startup. latinum-inflate is a proxy and
  // requires an upstream command — we now refuse the bare flag (#474).
  const r = run('--with-mcp-inflate', '--non-interactive', '--dry-run');
  assert.equal(r.status, 2);
  assert.match(r.stderr, /requires an upstream command/);
  assert.match(r.stderr, /server-filesystem/);
});

test('--with-mcp-inflate followed by another flag (no value) exits 2', () => {
  // The next-token form must distinguish "no value" from "value happens to
  // start with --". A user typing `--with-mcp-inflate --dry-run` clearly
  // forgot the upstream; refuse.
  const r = run('--with-mcp-inflate', '--dry-run', '--non-interactive');
  assert.equal(r.status, 2);
  assert.match(r.stderr, /requires an upstream command/);
});

test('--with-mcp-inflate="<cmd>" registers wrapping that upstream', () => {
  const r = run(
    '--with-mcp-inflate=npx @modelcontextprotocol/server-filesystem /tmp',
    '--only', 'claude', '--dry-run', '--non-interactive',
    '--config-dir', '/tmp/__cm_inflate_test'
  );
  assert.equal(r.status, 0);
  ...
  if (/would run: claude mcp add latinum-inflate/.test(r.stdout)) {
    assert.match(r.stdout, /claude mcp add latinum-inflate .* npx -y latinum-inflate npx @modelcontextprotocol\/server-filesystem \/tmp/);
  }
});

test('--with-mcp-inflate "<cmd>" (space-separated) also accepted', () => {
  const r = run(
    '--with-mcp-inflate', 'npx @modelcontextprotocol/server-filesystem /tmp',
    '--only', 'claude', '--dry-run', '--non-interactive',
    '--config-dir', '/tmp/__cm_inflate_space'
  );
  assert.equal(r.status, 0);
  if (/would run: claude mcp add latinum-inflate/.test(r.stdout)) {
    assert.match(r.stdout, /latinum-inflate npx @modelcontextprotocol\/server-filesystem \/tmp/);
  }
});

test('--all does NOT auto-enable mcp-inflate (no sensible default upstream)', () => {
  const r = run('--all', '--only', 'claude', '--dry-run', '--non-interactive', '--config-dir', '/tmp/__cm_all_no_inflate');
  assert.equal(r.status, 0);
  // Whether or not claude is on PATH, the wiring banner should not appear
  // because withMcpShrink stays false under --all alone.
  assert.doesNotMatch(r.stdout, /wiring latinum-inflate MCP proxy/);
});
