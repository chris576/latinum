...
// Claude Code accepts. Copying agents/latinumcrew-*.md verbatim into
// ~/.config/opencode/agents/ broke opencode startup with:
//   Configuration is invalid at .../latinumcrew-reviewer.md
...
const SHIPPED_AGENT_FILES = ['latinumcrew-investigator.md', 'latinumcrew-builder.md', 'latinumcrew-reviewer.md'];
...
// This is the RED-state proof: each `agents/latinumcrew-*.md` in the repo today
...
test('all shipped latinumcrew agent files contain offending tools array (RED proof)', () => {
...
test('all shipped latinumcrew agent files become opencode-safe after transform (GREEN proof)', () => {
  for (const f of SHIPPED_AGENT_FILES) {
    const src = fs.readFileSync(path.join(REPO_ROOT, 'agents', f), 'utf8');
    const out = stripOpencodeAgentTools(src);
    const fm = frontmatter(out);
    ...
    assert.match(fm, /^name: latinumcrew-/m, `${f}: name field preserved`);
