...
test('dry-run --only claude prints plan and writes nothing', () => {
  ...
  const r = spawnSync('node', [INSTALLER,
    ...
    '--dry-run', '--only', 'claude', '--with-hooks', '--no-mcp-inflate', '--non-interactive',
    '--config-dir', cfg,
  ...
  if (/Claude Code detected/.test(r.stdout)) {
    ...
    assert.match(r.stdout, /would run: claude plugin install latinum@latinum/);
    assert.match(r.stdout, /would mkdir -p .*\/hooks/);
    assert.match(r.stdout, /would install .*latinum-activate\.js/);
...
test('dry-run --uninstall does not delete files', () => {
  ...
  const fake = path.join(cfg, 'hooks', 'latinum-activate.js');
