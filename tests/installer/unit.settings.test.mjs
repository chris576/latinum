...
test('addCommandHook is idempotent on substring marker', () => {
  const s = {};
  const a = SETTINGS.addCommandHook(s, 'SessionStart', { command: '/abs/path/latinum-activate.js', marker: 'latinum-activate' });
  const b = SETTINGS.addCommandHook(s, 'SessionStart', { command: '/different/abs/path/latinum-activate.js', marker: 'latinum-activate' });
  assert.equal(a, true);
  assert.equal(b, false);
  assert.equal(s.hooks.SessionStart.length, 1);
});

test('hasLatinumHook detects via substring', () => {
  const s = { hooks: { SessionStart: [{ hooks: [{ type: 'command', command: 'node /x/latinum-activate.js' }] }] } };
  assert.equal(SETTINGS.hasLatinumHook(s, 'SessionStart', 'latinum-activate'), true);
  assert.equal(SETTINGS.hasLatinumHook(s, 'SessionStart', 'gsd'), false);
  assert.equal(SETTINGS.hasLatinumHook(s, 'UserPromptSubmit'), false);
});

test('removeLatinumHooks tolerates malformed hook event values without throwing', () => {
  ...
  let removed;
  assert.doesNotThrow(() => { removed = SETTINGS.removeLatinumHooks(s, 'latinum'); });
  assert.equal(removed, 0);
  assert.equal(s.hooks, undefined);
});

test('removeLatinumHooks strips by marker and cleans empties', () => {
  const s = {
    hooks: {
      SessionStart: [
        { hooks: [{ type: 'command', command: 'latinum-x' }] },
        { hooks: [{ type: 'command', command: 'other' }] },
      ],
      UserPromptSubmit: [{ hooks: [{ type: 'command', command: 'latinum-y' }] }],
    },
  };
  const removed = SETTINGS.removeLatinumHooks(s, 'latinum');
...
test('rewriteLegacyManagedHookCommands rewrites bare-node managed scripts', () => {
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        { type: 'command', command: 'node /abs/hooks/latinum-activate.js' },
  ...
  assert.equal(n, 1);
  assert.match(s.hooks.SessionStart[0].hooks[0].command, /"\/usr\/local\/bin\/node" "\/abs\/hooks\/latinum-activate\.js"/);
...
test('rewriteLegacyManagedHookCommands ignores already-absolute node commands', () => {
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        { type: 'command', command: '"/usr/local/bin/node" "/abs/hooks/latinum-activate.js"' },
      ] }],
    },
  };
...
test('pruneOrphanedManagedHooks removes managed hook whose target is missing (absolute-node)', () => {
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        { type: 'command', command: '"/opt/node/bin/node" "/no/such/dir/latinum-activate.js"' },
      ] }],
    },
  };
  ...
  const s = {
    hooks: {
      UserPromptSubmit: [{ hooks: [
        { type: 'command', command: 'node /no/such/dir/latinum-mode-tracker.js' },
      ] }],
    },
  };
  const removed = SETTINGS.pruneOrphanedManagedHooks(s, '/tmp/__cm_cfg_missing');
...
test('pruneOrphanedManagedHooks keeps managed hook whose target exists', () => {
  ...
  const script = path.join(dir, 'latinum-activate.js');
...
test('pruneOrphanedManagedHooks resolves relative target against configDir', () => {
  ...
  // hooks/latinum-activate.js intentionally NOT created → missing
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        { type: 'command', command: 'node hooks/latinum-activate.js' },
      ] }],
    },
  };
  const removed = SETTINGS.pruneOrphanedManagedHooks(s, dir);
...
test('pruneOrphanedManagedHooks does NOT match a user script whose name merely contains a managed basename', () => {
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        // basename is "mylatinum-activate.js" — not an exact managed basename
        { type: 'command', command: 'node /no/such/dir/mylatinum-activate.js' },
      ] }],
    },
  };
  const removed = SETTINGS.pruneOrphanedManagedHooks(s, '/tmp/__cm_cfg_missing');
...
test('pruneOrphanedManagedHooks drops orphaned managed statusLine', () => {
  const s = {
    statusLine: { type: 'command', command: 'bash /no/such/dir/latinum-statusline.sh' },
  };
  const removed = SETTINGS.pruneOrphanedManagedHooks(s, '/tmp/__cm_cfg_missing');
  assert.equal(removed, 1);
