...
function freshTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'latinum-opencode-'));
}
...
function runInstaller(args, env) {
  return spawnSync('node', [INSTALLER, ...args, '--non-interactive', '--no-mcp-inflate'], {
    env, encoding: 'utf8',
  });
}
...
test('opencode fresh install drops plugin, commands, agents, skills, AGENTS.md, opencode.json', () => {
  ...
  try {
    ...
    assert.ok(fs.existsSync(path.join(ocDir, 'plugins', 'latinum', 'plugin.js')), 'plugin.js missing');
    assert.ok(fs.existsSync(path.join(ocDir, 'plugins', 'latinum', 'package.json')), 'plugin package.json missing');
    assert.ok(fs.existsSync(path.join(ocDir, 'plugins', 'latinum', 'latinum-config.cjs')), 'latinum-config.cjs sibling missing');

    for (const f of ['latinum.md', 'latinum-commit.md', 'latinum-review.md', 'latinum-expand.md', 'latinum-stats.md', 'latinum-help.md']) {
      assert.ok(fs.existsSync(path.join(ocDir, 'commands', f)), `command ${f} missing`);
    }
    for (const f of ['latinumcrew-investigator.md', 'latinumcrew-builder.md', 'latinumcrew-reviewer.md']) {
      assert.ok(fs.existsSync(path.join(ocDir, 'agents', f)), `agent ${f} missing`);
    }
    for (const name of ['latinum', 'latinum-commit', 'latinum-review', 'latinum-help', 'latinum-stats', 'latinum-expand', 'latinumcrew']) {
    ...
    assert.match(agentsBody, /Respond terse like smart latinum/);
    // Block must be wrapped in begin/end markers so uninstall can isolate it
    // from user-authored content above and below.
    assert.match(agentsBody, /<!-- latinum-begin -->/);
    assert.match(agentsBody, /<!-- latinum-end -->/);
    ...
    assert.ok(cfg.plugin.includes('./plugins/latinum/plugin.js'), 'plugin entry missing');
  } finally {
...
test('opencode idempotent install does not duplicate plugin entries', () => {
  ...
  try {
    ...
    const matches = cfg.plugin.filter(p => p === './plugins/latinum/plugin.js');
    ...
    const sentinelCount = (agentsMd.match(/Respond terse like smart latinum/g) || []).length;
...
test('opencode re-install preserves user edits to plugin.js without --force', () => {
  ...
  try {
    ...
    const pluginPath = path.join(xdg, 'opencode', 'plugins', 'latinum', 'plugin.js');
...
test('opencode uninstall strips fenced AGENTS.md block, preserving user prefix and suffix', () => {
  ...
  try {
    ...
    // Sandwich the latinum block between user prefix and suffix.
    ...
    assert.doesNotMatch(after, /<!-- latinum-begin -->/, 'latinum block should be stripped');
    assert.doesNotMatch(after, /<!-- latinum-end -->/, 'latinum end marker should be stripped');
    assert.doesNotMatch(after, /Respond terse like smart latinum/, 'latinum body should be stripped');
...
test('opencode uninstall removes plugin dir, command/agent/skill files, prunes opencode.json', () => {
  ...
  try {
    ...
    const ocDir = path.join(xdg, 'opencode');
    assert.equal(fs.existsSync(path.join(ocDir, 'plugins', 'latinum')), false, 'plugin dir survived');
    assert.equal(fs.existsSync(path.join(ocDir, 'commands', 'latinum.md')), false, 'latinum.md command survived');
    assert.equal(fs.existsSync(path.join(ocDir, 'agents', 'latinumcrew-builder.md')), false, 'latinumcrew agent survived');
    assert.equal(fs.existsSync(path.join(ocDir, 'skills', 'latinum')), false, 'latinum skill dir survived');
    ...
    if (fs.existsSync(path.join(ocDir, 'opencode.json'))) {
      const cfg = JSON.parse(fs.readFileSync(path.join(ocDir, 'opencode.json'), 'utf8'));
      const stillHasPlugin = Array.isArray(cfg.plugin) && cfg.plugin.includes('./plugins/latinum/plugin.js');
...
test('opencode plugin handles /latinum ultra, stop latinum, and session init via real hooks', async () => {
  const xdg = freshTmpDir();
  const shimDir = shimOpencode();
  const origDefault = process.env.LATINUM_DEFAULT_MODE;
  try {
    const env = { ...process.env, XDG_CONFIG_HOME: xdg, PATH: pathWith(shimDir), NO_COLOR: '1' };
    const r = runInstaller(['--only', 'opencode'], env);
    assert.notEqual(r.status, 2);
    ...
    const pluginPath = path.join(xdg, 'opencode', 'plugins', 'latinum', 'plugin.js');
    const flagPath = path.join(xdg, 'opencode', '.latinum-active');
    ...
    // any ambient user/repo-local latinum config.
    process.env.XDG_CONFIG_HOME = xdg;
    process.env.LATINUM_DEFAULT_MODE = 'maxi';

    const mod = await import(pathToFileURL(pluginPath).href);
    const factory = mod.default || mod.LatinumPlugin;
    ...
    await handlers['chat.message']({}, { parts: [{ type: 'text', text: '/latinum ultra' }] });
    assert.equal(fs.readFileSync(flagPath, 'utf8'), 'ultra');

    // opencode expands "/latinum <level>" into the command template before
    // chat.message fires — the level must be recovered from the expanded text.
    await handlers['chat.message']({}, { parts: [{ type: 'text', text:
      'Activate latinum mode: wenyan-premium\n\nIf no level given, use maxi. If "off", deactivate.' }] });
    assert.equal(fs.readFileSync(flagPath, 'utf8'), 'wenyan-premium');
    await handlers['chat.message']({}, { parts: [{ type: 'text', text:
      'Activate latinum mode: off\n\nIf no level given, use maxi. If "off", deactivate.' }] });
    assert.equal(fs.existsSync(flagPath), false, 'expanded template with off should delete the flag');
    await handlers['chat.message']({}, { parts: [{ type: 'text', text:
      'Activate latinum mode: \n\nIf no level given, use maxi. If "off", deactivate.' }] });
    assert.equal(fs.readFileSync(flagPath, 'utf8'), 'maxi', 'expanded template without level uses default');
    await handlers['chat.message']({}, { parts: [{ type: 'text', text: '/latinum ultra' }] });
    assert.equal(fs.readFileSync(flagPath, 'utf8'), 'ultra');

    // opencode's non-interactive `run` path wraps the message in literal
    // quotes ("/latinum premium"\n) — the parser must unwrap them.
    await handlers['chat.message']({}, { parts: [{ type: 'text', text: '"/latinum premium"\n' }] });
    assert.equal(fs.readFileSync(flagPath, 'utf8'), 'premium');
    await handlers['chat.message']({}, { parts: [{ type: 'text', text: '/latinum ultra' }] });
    assert.equal(fs.readFileSync(flagPath, 'utf8'), 'ultra');
    ...
    assert.match(sys1.system[0], /LATINUM MODE ACTIVE \(ultra\)/);

    // Natural-language deactivation removes the flag.
    await handlers['chat.message']({}, { parts: [{ type: 'text', text: 'stop latinum please' }] });
    ...
    assert.equal(fs.readFileSync(flagPath, 'utf8'), 'maxi');
  } finally {
    if (origDefault === undefined) delete process.env.LATINUM_DEFAULT_MODE;
    else process.env.LATINUM_DEFAULT_MODE = origDefault;
    fs.rmSync(xdg, { recursive: true, force: true });
    fs.rmSync(shimDir, { recursive: true, force: true });
  }
});
