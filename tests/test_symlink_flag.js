...
const { safeWriteFlag, readFlag, VALID_MODES } = require('../src/hooks/latinum-config');

let passed = 0;
let failed = 0;

function test(name, fn) {
  const tmpBase = fs.mkdtempSync(path.join(os.tmpdir(), 'latinum-symlink-test-'));
  try {
    fn(tmpBase);
    passed++;
    console.log(`  ✓ ${name}`);
...
test('writes flag in normal (non-symlinked) directory', (tmp) => {
  ...
  const flagPath = path.join(flagDir, '.latinum-active');

  safeWriteFlag(flagPath, 'maxi');

  assert.strictEqual(fs.readFileSync(flagPath, 'utf8'), 'maxi');
});
...
test('writes flag when parent directory is a symlink owned by current user', (tmp) => {
  ...
  const flagPath = path.join(symlinkDir, '.latinum-active');
  safeWriteFlag(flagPath, 'ultra');

  // Flag should exist in the real directory
  const realFlagPath = path.join(realDir, '.latinum-active');
...
test('readFlag works through symlinked parent directory', (tmp) => {
  ...
  const realFlagPath = path.join(realDir, '.latinum-active');
  fs.writeFileSync(realFlagPath, 'premium', { mode: 0o600 });

  const result = readFlag(path.join(symlinkDir, '.latinum-active'));
  assert.strictEqual(result, 'premium');
});
...
test('refuses flag file that is itself a symlink (even through symlinked parent)', (tmp) => {
  ...
  safeWriteFlag(path.join(symlinkDir, '.latinum-active'), 'maxi');
...
test('readFlag refuses flag file that is a symlink', (tmp) => {
  ...
  fs.symlinkSync(secretFile, path.join(realDir, '.latinum-active'));

  const result = readFlag(path.join(realDir, '.latinum-active'));
...
test('overwrites existing flag through symlinked parent', (tmp) => {
  ...
  safeWriteFlag(flagPath, 'premium');
  assert.strictEqual(readFlag(flagPath), 'premium');
...
test('symlink to nonexistent target silently fails', (tmp) => {
  ...
  assert.strictEqual(fs.existsSync(path.join(symlinkDir, '.latinum-active')), false);
});
...
test('safeWriteFlag no longer has blanket symlink parent refusal', (tmp) => {
  ...
  const source = fs.readFileSync(
    path.join(__dirname, '..', 'src', 'hooks', 'latinum-config.js'), 'utf8'
  );
