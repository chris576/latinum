#!/usr/bin/env python3
"""Local verification runner for latinum install surfaces."""
...
def verify_skill_frontmatter_upload_compatibility() -> None:
    ...
    skill_paths = [
        ROOT / "skills/latinum/SKILL.md",
        ROOT / "skills/latinum-commit/SKILL.md",
        ROOT / "skills/latinum-help/SKILL.md",
        ROOT / "skills/latinum-review/SKILL.md",
        ROOT / "skills/latinum-expand/SKILL.md",
    ]
...
def verify_synced_files() -> None:
    ...
    skill_source = ROOT / "skills/latinum/SKILL.md"

    skill_copies = [
        ROOT / "plugins/latinum/skills/latinum/SKILL.md",
    ]
    ...
    with zipfile.ZipFile(ROOT / "dist" / "latinum.skill") as archive:
        ensure("latinum/SKILL.md" in archive.namelist(), "latinum.skill missing latinum/SKILL.md")
        ensure(
            archive.read("latinum/SKILL.md").decode("utf-8")
            == skill_source.read_text(encoding="utf-8"),
            "latinum.skill payload mismatch",
        )

    ensure(
        (ROOT / "bin" / "install.js").exists(),
        "bin/install.js missing — package.json bin entry would break npx latinum",
    )
    ensure(
    ...
    print("Synced copies, latinum.skill zip, and installer entrypoints OK")
...
def verify_manifests_and_syntax() -> None:
    ...
    manifest_paths = [
        ...
        ROOT / "plugins/latinum/.codex-plugin/plugin.json",
    ]
    for path in manifest_paths:
        read_json(path)

    run(["node", "--check", "src/hooks/latinum-config.js"])
    run(["node", "--check", "src/hooks/latinum-activate.js"])
    run(["node", "--check", "src/hooks/latinum-mode-tracker.js"])
    ...
    run(["bash", "-n", "src/hooks/latinum-statusline.sh"])

    # Ensure install/uninstall scripts include latinum-config.js
    ...
    ensure("latinum-config.js" in install_sh, "install.sh missing latinum-config.js")
    ensure("latinum-config.js" in uninstall_sh, "uninstall.sh missing latinum-config.js")
...
def verify_powershell_static() -> None:
    ...
    statusline_text = (ROOT / "src/hooks/latinum-statusline.ps1").read_text(encoding="utf-8")

    ensure("latinum-config.js" in install_text, "install.ps1 missing latinum-config.js")
    ensure("latinum-config.js" in uninstall_text, "uninstall.ps1 missing latinum-config.js")
    ensure("latinum-statusline.ps1" in install_text, "install.ps1 missing statusline.ps1")
    ensure("latinum-statusline.ps1" in uninstall_text, "uninstall.ps1 missing statusline.ps1")
    ...
    ensure("[LATINUM" in statusline_text, "latinum-statusline.ps1 missing badge output")

    print("Windows install path statically wired")


def load_compress_modules():
    sys.path.insert(0, str(ROOT / "skills/latinum-expand"))
...
def verify_compress_fixtures() -> None:
    ...
    fixtures = sorted((ROOT / "tests/latinum-expand").glob("*.original.md"))
    ensure(fixtures, "No latinum-expand fixtures found")
    ...
    print(f"Validated {len(fixtures)} latinum-expand fixture pairs")
...
def verify_compress_cli() -> None:
    ...
    skip_result = run(
        ...
        cwd=ROOT / "skills/latinum-expand",
        check=False,
    )
...
def verify_hook_install_flow() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="latinum-verify-") as temp_root:
        ...
        activate = run(
            ["node", "src/hooks/latinum-activate.js"],
            env=hook_env,
        )
        ensure("LATINUM MODE ACTIVE" in activate.stdout, "activation output missing latinum banner")
        ensure("STATUSLINE SETUP NEEDED" not in activate.stdout, "activation should stay quiet when custom statusline exists")
        ensure((claude_dir / ".latinum-active").read_text(encoding="utf-8") == "maxi", "activation flag should default to full")

        # Test configurable default mode via LATINUM_DEFAULT_MODE env var
        activate_custom = run(
            ["node", "src/hooks/latinum-activate.js"],
            env={**hook_env, "LATINUM_DEFAULT_MODE": "ultra"},
        )
        ensure("LATINUM MODE ACTIVE" in activate_custom.stdout, "activation with custom default missing banner")
        ensure(
            (claude_dir / ".latinum-active").read_text(encoding="utf-8") == "ultra",
            "LATINUM_DEFAULT_MODE=ultra should set flag to ultra",
        )
        # Test "off" mode — activation skipped, flag removed
        activate_off = run(
            ["node", "src/hooks/latinum-activate.js"],
            env={**hook_env, "LATINUM_DEFAULT_MODE": "off"},
        )
        ensure("LATINUM MODE ACTIVE" not in activate_off.stdout, "off mode should not emit latinum banner")
        ensure(not (claude_dir / ".latinum-active").exists(), "off mode should remove flag file")

        # Test mode tracker with /latinum when default is off — should NOT write flag
        subprocess.run(
            ["node", "src/hooks/latinum-mode-tracker.js"],
            cwd=ROOT,
            env={**os.environ, **hook_env, "LATINUM_DEFAULT_MODE": "off"},
            text=True,
            encoding="utf-8",
            input='{"prompt":"/latinum"}',
            capture_output=True,
            check=True,
        )
        ensure(not (claude_dir / ".latinum-active").exists(), "/latinum with off default should not write flag")

        # Reset back to full for subsequent tests
        (claude_dir / ".latinum-active").write_text("maxi")
        ...
        ultra_prompt = subprocess.run(
            ...
            text=True,
            encoding="utf-8",
            input='{"prompt":"/latinum ultra"}',
            capture_output=True,
            check=True,
        )
        ensure(
            "LATINUM MODE ACTIVE (ultra)" in ultra_prompt.stdout,
            "mode tracker should emit active-mode reinforcement",
        )
        ensure((claude_dir / ".latinum-active").read_text(encoding="utf-8") == "ultra", "mode tracker did not record ultra")
        ...
        ensure(not (claude_dir / ".latinum-active").exists(), "normal mode should remove flag file")

        (claude_dir / ".latinum-active").write_text("wenyan-ultra")
        statusline = run(
            ["bash", "src/hooks/latinum-statusline.sh"],
            env=hook_env,
        )
        ensure("[LATINUM:WENYAN-ULTRA]" in statusline.stdout, "statusline badge output mismatch")
        ...
        ensure(settings_after == existing_settings, "uninstall.sh did not restore non-latinum settings")
        ensure(not (claude_dir / ".latinum-active").exists(), "uninstall.sh should remove flag file")

    with tempfile.TemporaryDirectory(prefix="latinum-verify-fresh-") as temp_root:
        ...
        activate = run(["node", "src/hooks/latinum-activate.js"], env=hook_env)
