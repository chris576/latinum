#!/usr/bin/env node
// Tests for src/mcp-servers/latinum-inflate/compress.js — pure-Node prose compressor.
// Run: node tests/test_mcp_inflate.js
...
const { compress, compressDescriptionsInPlace } = require(
  path.join(ROOT, 'src', 'mcp-servers', 'latinum-inflate', 'compress.js')
);
const { getSpawnOptions } = require(
  path.join(ROOT, 'src', 'mcp-servers', 'latinum-inflate', 'spawn-options.js')
);
...
console.log('mcp-inflate compress tests\n');
