...
`latinum-expand` receives a Snyk High Risk rating due to static analysis heuristics. This document explains what the skill does and does not do.
...
2. **File read/write**: The skill reads the file the user explicitly points it at, expands it, and writes the result back to the same path. A `.original.md` backup is saved alongside it. No files outside the user-specified path are read or written.
...
- Does not collect or transmit any data beyond the file being expanded
