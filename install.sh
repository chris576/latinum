...
#   curl -fsSL https://raw.githubusercontent.com/chris576/latinum/main/install.sh | bash -s -- --all
#
# Local clone:
...
if [ "$NODE_MAJOR" -lt 18 ]; then
  echo "latinum: Node $NODE_MAJOR too old. Need Node ≥18." >&2
...
if ! command -v npx >/dev/null 2>&1; then
  echo "latinum: npx required (ships with Node ≥18). Reinstall Node.js." >&2
  exit 1
fi
