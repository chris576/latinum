// latinum — JSONC-tolerant settings.json read/write + defensive hook validation.
//
// Lifted in spirit from gsd-build/get-shit-done's stripJsonComments + readSettings.
// Reused by bin/install.js and (optionally) by hooks/latinum-activate.js so a
...
//   hasLatinumHook(settings, ev)   → idempotency probe
//   addCommandHook(settings, ev, opts) → no-op if substring marker already present
//   removeLatinumHooks(settings)   → uninstall helper
//
...
function readSettings(p) {
  ...
  try { raw = fs.readFileSync(p, 'utf8'); }
    ...
    process.stderr.write(`latinum: cannot read ${p}: ${e.message}\n`);
    return null;
  }
  if (!raw.trim()) return {};
  ...
  try { return JSON.parse(stripJsonComments(raw)); }
    ...
    process.stderr.write(`latinum: warning — ${p} is not valid JSON or JSONC: ${e.message}\n`);
    return null;
  }
}
...
// ── Idempotency probe ──────────────────────────────────────────────────────
function hasLatinumHook(settings, event, marker = 'latinum') {
...
function addCommandHook(settings, event, opts) {
  ...
  if (hasLatinumHook(settings, event, marker)) return false;
...
function removeLatinumHooks(settings, marker = 'latinum') {
...
// on user-authored hooks that just happen to mention "latinum".
...
// latinum-*.js) to the Claude Code plugin disables/renames those local
...
// …latinum-activate.js` (issue #471). rewriteLegacyManagedHookCommands can't
// help — it only matches the bare-node shape and these orphans are usually
// absolute-node — and removeLatinumHooks runs only on uninstall.
//
...
function pruneOrphanedManagedHooks(settings, configDir) {
  ...
  // `mylatinum-activate.js` is never touched) and that resolved path is absent.
