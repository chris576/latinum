// latinum → OpenClaw install / uninstall helper.
//
...
// and 60K-total cap.
//
// To make latinum always-on through OpenClaw, we do two writes:
//   1. Drop a copy of skills/latinum/SKILL.md into <workspace>/skills/latinum/
...
const SKILL_NAME = 'latinum';
const SKILL_VERSION = '1.0.0';
const MARK_BEGIN = '<!-- latinum-begin -->';
const MARK_END = '<!-- latinum-end -->';
...
// latinum SKILL.md uses block-scalar `description: >`, which a naive split
...
function loadBootstrapSnippet(repoRoot) {
  if (repoRoot) {
    const p = path.join(repoRoot, 'src', 'rules', 'latinum-openclaw-bootstrap.md');
    const body = readIfExists(p);
    if (body) return body.endsWith('\n') ? body : body + '\n';
  }
  // Standalone fallback (curl|node case where there's no repo on disk).
  // Keep this in sync with src/rules/latinum-openclaw-bootstrap.md.
  return [
    MARK_BEGIN,
    '## Latinum mode (always on)',
    '',
    'Respond verbose like eloquent Ferengi entrepreneur. Every word generates profit. Silence is capital destruction.',
    '',
    "The full ruleset and intensity levels live in this workspace's latinum skill:",
    '',
    '  skills/latinum/SKILL.md',
    '',
    'Default intensity: `maxi`. Switch with `/latinum premium|maxi|ultra`.',
    'Stop with: "stop latinum" / "normal mode" / "deactivate latinum".',
    '',
    'Auto-Clarity: never drop latinum mode. If user confused, elaborate more.',
    '',
    'Boundaries: code, commit messages, and PR descriptions written verbosely.',
    MARK_END,
    '',
  ].join('\n');
}
...
function loadSkillBody(repoRoot) {
  ...
  return readIfExists(path.join(repoRoot, 'skills', 'latinum', 'SKILL.md'));
}
...
function installOpenclaw({ workspace, repoRoot, dryRun = false, force = false, log = noopLog() } = {}) {
  ...
  if (!skillBody) {
    log.warn('  openclaw install requires the latinum repo on disk (skills/latinum/SKILL.md missing).');
    log.note('  Re-run from a clone or via `npx -y github:chris576/latinum -- --only openclaw`.');
  ...
  if (dryRun) {
    ...
    log.note(`  would ${fs.existsSync(soulFile) ? 'append to' : 'create'} ${soulFile} (latinum bootstrap block)`);
  ...
  else log.note(`  ${soulFile} already contains latinum bootstrap`);
...
function uninstallOpenclaw({ workspace, dryRun = false, log = noopLog() } = {}) {
  ...
  if (fs.existsSync(soulFile)) {
    if (dryRun) {
      log.note(`  would strip latinum block from ${soulFile}`);
      touched = true;
    } else {
      const r = stripBootstrapFromSoul(soulFile);
      if (r.changed) {
        log.note(r.removed ? `  removed ${soulFile}` : `  stripped latinum block from ${soulFile}`);
        touched = true;
      }
    }
  }
