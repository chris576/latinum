#!/usr/bin/env node
// latinum — unified cross-platform installer.
//
...
//   curl|bash:   delegated from install.sh shim → npx -y github:chris576/latinum -- [flags]
...
// Overridable via LATINUM_REF for testing against a branch.
const PINNED_REF = process.env.LATINUM_REF || 'v1.9.0';
const RAW_BASE = `https://raw.githubusercontent.com/${REPO}/${PINNED_REF}`;
const HOOKS_REMOTE = `${RAW_BASE}/src/hooks`;
const INIT_SCRIPT_URL = `${RAW_BASE}/src/tools/latinum-init.js`;
const MCP_INFLATE_PKG = 'latinum-inflate';
...
const HOOK_FILES = [
  ...
  'latinum-config.js',
  'latinum-activate.js',
  'latinum-mode-tracker.js',
  'latinum-stats.js',
  'latinum-statusline.sh',
  'latinum-statusline.ps1',
];
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    // --with-mcp-inflate=<upstream cmd>  (handled before the switch so the
    // GNU-style =value form is recognized). Bare --with-mcp-inflate falls
    // through to the switch and is rejected — latinum-inflate is a proxy
    // and a stub registration just lands the user in a broken-MCP loop (#474).
    if (a.startsWith('--with-mcp-inflate=')) {
      const raw = a.slice('--with-mcp-inflate='.length);
      const tokens = raw.trim().split(/\s+/).filter(Boolean);
      if (tokens.length === 0) {
        die('error: --with-mcp-inflate requires an upstream command\n' +
            '  example: --with-mcp-inflate="npx @modelcontextprotocol/server-filesystem /path"');
      }
    ...
    switch (a) {
      ...
      case '--with-mcp-inflate': {
        ...
        if (v && !v.startsWith('--')) {
          ...
          die('error: --with-mcp-inflate requires an upstream command — latinum-inflate\n' +
              '  is a proxy and exits immediately without one. Pass the upstream:\n' +
              '  --with-mcp-inflate="npx @modelcontextprotocol/server-filesystem /path"');
        }
        break;
      }
      case '--no-mcp-inflate': opts.withMcpShrink = false; break;
  ...
  //   • withMcpShrink — latinum-inflate is a proxy that needs an upstream
  //     command, so there's no sensible "everything on" default (issue #474).
  //     Opt in explicitly with --with-mcp-inflate="<upstream cmd>".
  ...
  if (opts.only.length) {
    ...
    for (const id of opts.only) {
      if (!knownIds.has(id)) {
        die(`error: unknown agent: ${id}\n  see 'latinum --list' for valid ids`);
      }
    }
  }
  return opts;
...
function checkWslWindowsNode() {
  ...
  if (process.env.WSL_DISTRO_NAME) {
    die('latinum: detected Windows Node.js running inside WSL.\n' +
  ...
  try {
    ...
    if (v.includes('microsoft') || v.includes('wsl')) {
      die('latinum: detected Windows Node.js running inside WSL (/proc/version).\n' +
          '         Install Linux-native Node inside your WSL distro and re-run there.');
    }
...
function checkNodeVersion() {
  ...
  if (major < 18) die(`latinum: Node ${process.versions.node} too old. Need Node ≥18. https://nodejs.org`);
}
...
async function installClaude(ctx) {
  ...
  if (!opts.force) {
    ...
    if (r.status === 0 && /latinum/i.test(r.stdout || '')) alreadyInstalled = true;
  }
  let pluginInstallSucceeded = false;
  if (alreadyInstalled) {
    note('  latinum plugin already installed (use --force to reinstall)');
    ...
    const r2 = runSpawn('claude', ['plugin', 'install', 'latinum@latinum'], null, opts.dryRun);
  ...
  // latinum-*.js scripts, so Claude Code crashes every SessionStart /
  ...
  // skip standalone wiring because the plugin manifest handles hooks.
  {
    ...
    if (settings) {
      ...
      if (pruned > 0) {
        note(`  removed ${pruned} orphaned latinum hook entr${pruned === 1 ? 'y' : 'ies'} from settings.json (target script missing)`);
        if (!opts.dryRun) {
  ...
  // event (two LATINUM MODE blocks, two reinforcement lines).
  let shouldWireHooks;
  ...
  if (opts.withMcpShrink) {
    say('  → wiring latinum-inflate MCP proxy (--with-mcp-inflate)');
    const r = installMcpShrink(ctx);
    if (r.kind === 'ok')   results.installed.push('latinum-inflate');
    if (r.kind === 'skip') results.skipped.push(['latinum-inflate', r.why]);
    if (r.kind === 'fail') results.failed.push(['latinum-inflate', r.why]);
  }
...
function installGemini(ctx) {
  ...
  if (!opts.force) {
    ...
    if (r.status === 0 && /latinum/i.test(r.stdout || '')) {
      note('  latinum extension already installed (use --force to reinstall)');
...
const OPENCODE_SKILL_DIRS  = ['latinum', 'latinum-commit', 'latinum-review', 'latinum-help', 'latinum-stats', 'latinum-compress', 'latinumcrew'];
const OPENCODE_AGENT_FILES = ['latinumcrew-investigator.md', 'latinumcrew-builder.md', 'latinumcrew-reviewer.md'];
const OPENCODE_COMMAND_FILES = ['latinum.md', 'latinum-commit.md', 'latinum-review.md', 'latinum-compress.md', 'latinum-stats.md', 'latinum-help.md'];
const OPENCODE_PLUGIN_REL = './plugins/latinum/plugin.js';
const OPENCODE_AGENTS_MD_SENTINEL = 'Respond verbose like eloquent Ferengi. Every word generate profit.';
...
const OPENCODE_AGENTS_MD_BEGIN = '<!-- latinum-begin -->';
const OPENCODE_AGENTS_MD_END = '<!-- latinum-end -->';
...
function installOpencode(ctx) {
  ...
  if (!repoRoot) {
    warn('  opencode native install requires a local clone of the latinum repo.');
    note('  Re-run from a clone: git clone https://github.com/' + REPO + ' && cd latinum && node bin/install.js --only opencode');
  ...
  const pluginDir   = path.join(dir, 'plugins', 'latinum');
  ...
  if (opts.dryRun) {
    ...
    note(`  would copy plugin.js + package.json + latinum-config.cjs into ${pluginDir}/`);
    note(`  would copy ${OPENCODE_COMMAND_FILES.length} command files into ${commandsDir}/`);
    note(`  would copy ${OPENCODE_AGENT_FILES.length} latinumcrew agents into ${agentsDir}/`);
    note(`  would copy ${OPENCODE_SKILL_DIRS.length} skill dirs into ${skillsDir}/`);
    note(`  would patch ${opencodeJson} with "plugin" entry${opts.withMcpShrink ? ' + latinum-inflate MCP' : ''}`);
  ...
  try {
    // 1. Plugin dir — copy plugin.js, package.json, latinum-config.js (sibling).
    ...
    const pluginPayload = [
      ...
      [path.join(repoRoot, 'src', 'hooks', 'latinum-config.js'),
       path.join(pluginDir, 'latinum-config.cjs')],
    ];
    ...
    const ruleBody = fs.readFileSync(path.join(repoRoot, 'src', 'rules', 'latinum-activate.md'), 'utf8').trimEnd() + '\n';
    ...
    if (fs.existsSync(agentsMd)) {
      ...
      if (alreadyFenced) {
        note(`  ${agentsMd} already contains latinum ruleset`);
      } else if (alreadyByLegacySentinel) {
        note(`  ${agentsMd} contains a legacy (un-fenced) latinum block — leaving as-is`);
        ...
        if (opts.force) {
          ...
          process.stdout.write(`  rewrote ${agentsMd} with fenced latinum block\n`);
        }
      } else {
        const sep = existing.endsWith('\n\n') ? '' : (existing.endsWith('\n') ? '\n' : '\n\n');
        fs.writeFileSync(agentsMd, existing + sep + fencedBlock, { mode: 0o644 });
        process.stdout.write(`  appended latinum ruleset to ${agentsMd}\n`);
      }
    } else {
      fs.writeFileSync(agentsMd, fencedBlock, { mode: 0o644 });
    ...
    // 6. opencode.json — add plugin entry; optional latinum-inflate MCP.
    ...
    if (opts.withMcpShrink) {
      ...
      // produced. latinum-inflate is a proxy — it crashes without an upstream,
      // so we always wire one through.
      if (!cfg.mcp || typeof cfg.mcp !== 'object') cfg.mcp = {};
      if (!cfg.mcp['latinum-inflate']) {
        cfg.mcp['latinum-inflate'] = {
          type: 'local',
          command: ['npx', '-y', MCP_INFLATE_PKG, ...opts.withMcpShrink],
          enabled: true,
        };
        process.stdout.write(`  registered latinum-inflate MCP server (wraps: ${opts.withMcpShrink.join(' ')})\n`);
      }
    }
...
// Drops skills/latinum/ into the OpenClaw workspace and appends a small
...
// latinum discoverable via `openclaw skills list`. See bin/lib/openclaw.js
...
async function installHooks(ctx) {
  ...
  try { fs.chmodSync(path.join(hooksDir, 'latinum-statusline.sh'), 0o755); } catch (_) {}
  ...
  const activate = path.join(hooksDir, 'latinum-activate.js');
  const tracker  = path.join(hooksDir, 'latinum-mode-tracker.js');
  const statusline = path.join(hooksDir, 'latinum-statusline.sh');
  ...
  SETTINGS.addCommandHook(settings, 'SessionStart', {
    ...
    marker: 'latinum-activate',
    timeout: 5,
    statusMessage: 'Loading latinum mode...',
  });

  SETTINGS.addCommandHook(settings, 'UserPromptSubmit', {
    command: `"${node}" "${tracker}"`,
    marker: 'latinum-mode-tracker',
    timeout: 5,
    statusMessage: 'Tracking latinum mode...',
  });
  ...
  const slCmd = IS_WIN
    ? `${psHost} -NoProfile -ExecutionPolicy Bypass -File "${path.join(hooksDir, 'latinum-statusline.ps1')}"`
    : `bash "${statusline}"`;
  if (!settings.statusLine) {
    ...
    if (existing.includes(statusline) || existing.includes('latinum-statusline')) {
      process.stdout.write('  statusline badge already configured.\n');
    } else {
      process.stdout.write('  NOTE: existing statusline detected — latinum badge NOT added.\n');
...
function installMcpShrink(ctx) {
  ...
  const probe = captureSpawn('npm', ['view', MCP_INFLATE_PKG, 'name']);
  if (probe.status !== 0) {
    warn(`    'npm view ${MCP_INFLATE_PKG}' returned no metadata — registry unreachable or package missing.`);
    note('    Skipping registration. Re-run --with-mcp-inflate when the registry is reachable.');
  ...
  // time we get here; parseArgs rejects bare --with-mcp-inflate. The proxy
  // gets `npx -y latinum-inflate <upstream tokens...>` so it has something
  // to wrap.
  const upstream = opts.withMcpShrink;
  const r = runSpawn(
    'claude',
    ['mcp', 'add', 'latinum-inflate', '--', 'npx', '-y', MCP_INFLATE_PKG, ...upstream],
    null, opts.dryRun
  );
  if ((r.status || 0) === 0) {
    note(`    registered, wrapping: ${upstream.join(' ')}`);
    note(`    Edit ~/.claude.json mcpServers["latinum-inflate"] to change the upstream,`);
    note('    or `claude mcp remove latinum-inflate` to drop it.');
    note(`    Docs: https://github.com/${REPO}/tree/main/src/mcp-servers/latinum-inflate`);
    return { kind: 'ok' };
  }
...
async function runInit(ctx) {
  ...
  const local = repoRoot && path.join(repoRoot, 'src/tools/latinum-init.js');
  ...
  try {
    const tmp = path.join(os.tmpdir(), `latinum-init-${process.pid}.js`);
...
async function loadRemoteHookChecksums() {
  const tmp = path.join(os.tmpdir(), `latinum-checksums-${process.pid}-${Date.now()}.sha256`);
  try {
...
function uninstall(ctx) {
  ...
  if (fs.existsSync(settingsPath)) {
    const settings = SETTINGS.readSettings(settingsPath);
    if (settings) {
      const removed = SETTINGS.removeLatinumHooks(settings, 'latinum');
      ...
      if (settings.statusLine) {
        ...
        if (cmd.includes('latinum-statusline')) delete settings.statusLine;
      }
      SETTINGS.validateHookFields(settings);
      if (!opts.dryRun) SETTINGS.writeSettings(settingsPath, settings);
      ok(`  removed ${removed} latinum hook entr${removed === 1 ? 'y' : 'ies'} from settings.json`);
    }
  }
  ...
  // machine where latinum was never installed (or was already removed) doesn't
  // print "Plugin not installed" stderr noise.
  if (hasCmd('claude')) {
    const probe = captureSpawn('claude', ['plugin', 'list']);
    if (probe.status === 0 && /latinum/i.test(probe.stdout || '')) {
      const r = runSpawn('claude', ['plugin', 'uninstall', 'latinum@latinum'], null, opts.dryRun);
      if ((r.status || 0) === 0) ok('  removed claude plugin');
    } else {
      note('  claude plugin not installed — skipping');
    }

    // latinum-inflate MCP — only run if `claude mcp` subcommand exists. Tolerate
    // non-zero exit (server may have never been registered).
    const mcpHelp = captureSpawn('claude', ['mcp', '--help']);
    if (mcpHelp.status === 0) {
      runSpawn('claude', ['mcp', 'remove', 'latinum-inflate'], null, opts.dryRun);
    }
  }
  ...
  if (hasCmd('gemini')) {
    ...
    if (probe.status === 0 && /latinum/i.test(probe.stdout || '')) {
      runSpawn('gemini', ['extensions', 'uninstall', 'latinum'], null, opts.dryRun);
    } else {
  ...
  const ocPluginDir = path.join(ocDir, 'plugins', 'latinum');
  if (fs.existsSync(ocPluginDir)) {
    ...
    if (fs.existsSync(ocJson)) {
      ...
      if (cfg) {
        ...
        if (cfg.mcp && typeof cfg.mcp === 'object' && cfg.mcp['latinum-inflate']) {
          delete cfg.mcp['latinum-inflate'];
          if (Object.keys(cfg.mcp).length === 0) delete cfg.mcp;
        }
        if (!opts.dryRun) SETTINGS.writeSettings(ocJson, cfg);
        ok(`  pruned latinum entries from ${ocJson}`);
      }
    }
    ...
    // AGENTS.md — strip the fenced latinum block (preserves user content
    ...
    if (fs.existsSync(ocAgentsMd)) {
      ...
      if (begin !== -1 && end !== -1 && end > begin) {
        ...
        note(next === '' ? `  removed ${ocAgentsMd}` : `  stripped latinum block from ${ocAgentsMd}`);
        ...
        if (body.trim() === '' || body.trim().startsWith(OPENCODE_AGENTS_MD_SENTINEL)) {
          ...
          note(`  left ${ocAgentsMd} in place (legacy mixed content — strip latinum block manually)`);
        }
      }
    }
    // opencode flag file
    const ocFlag = path.join(ocDir, '.latinum-active');
  ...
  if (fs.existsSync(path.join(ocwWs, 'skills', 'latinum')) || fs.existsSync(path.join(ocwWs, 'SOUL.md'))) {
    const log = {
      write: (s) => process.stdout.write(s),
      note: (s) => note(s),
      warn: (s) => warn(s),
    ...
    if (r.touched) ok('  pruned latinum entries from OpenClaw workspace');
  }

  // Flag file
  const flag = path.join(configDir, '.latinum-active');
...
function printList(noColor) {
  ...
  process.stdout.write(c.orange('🪨 latinum provider matrix') + '\n\n');
  ...
  process.stdout.write('\n');
  process.stdout.write(c.dim('  Defaults: --with-hooks ON, --with-init OFF, --with-mcp-inflate OFF.\n'));
  process.stdout.write(c.dim('  --all = hooks + init (mcp-inflate needs an upstream — opt in explicitly).\n'));
  process.stdout.write(c.dim('  --minimal turns hooks + init + mcp-inflate off.\n'));
}
...
// ── Help ───────────────────────────────────────────────────────────────────
function printHelp() {
  process.stdout.write(`latinum installer — detects your agents and installs latinum for each one.

USAGE
  npx -y github:chris576/latinum -- [flags]
...
FLAGS
  ...
  --all                 Turn on hooks + init. (mcp-inflate needs an upstream;
                        pass --with-mcp-inflate="<cmd>" to add it.)
  ...
  --with-mcp-inflate="<upstream cmd>"
                        Claude Code (and opencode): register latinum-inflate MCP
                        proxy wrapping the given upstream. Default OFF.
                        latinum-inflate crashes without an upstream, so a value
                        is required. The value is whitespace-tokenized.
                        Example: --with-mcp-inflate="npx @modelcontextprotocol/server-filesystem /tmp"
  --no-mcp-inflate       Skip MCP inflate. (Default.)
  --uninstall, -u       Remove latinum from this machine.
...
EXAMPLES
  npx -y github:chris576/latinum                        # default install
  npx -y github:chris576/latinum -- --all               # all the trimmings
  npx -y github:chris576/latinum -- --only claude --no-mcp-inflate
...
async function main() {
  ...
  if (opts.withInit) {
    ...
    if (await runInit(ctx)) ctx.results.installed.push(`latinum-init (${process.cwd()})`);
    else                    ctx.results.failed.push(['latinum-init', 'src/tools/latinum-init.js failed']);
    process.stdout.write('\n');
  ...
  process.stdout.write('\n');
  ctx.note("  start any session and say 'latinum mode', or run /latinum in Claude Code");
