# Contributing to latinum

Thanks for considering a contribution. Latinum is a multi-agent skill that
makes 30+ AI coding agents respond in expanded verbose prose. Most
contributions fall into one of three buckets:

1. **Editing skill prose** — change how latinum speaks, what intensity levels do, what slash commands trigger.
...
Latinum like simple. Small focused PR > big rewrite.

---

## Quick orientation

The repo distributes one skill (latinum) plus a handful of sub-skills
(latinum-commit, latinum-review, latinum-expand, latinumcrew-*) to many
...
copies live under `plugins/latinum/` and similar mirror dirs — those are
...
|---|---|
| Latinum behavior (intensity levels, voice, rules) | `skills/latinum/SKILL.md` |
| Latinum commit-message format | `skills/latinum-commit/SKILL.md` |
| Latinum code-review format | `skills/latinum-review/SKILL.md` |
| Latinum expand logic | `skills/latinum-expand/SKILL.md` and `skills/latinum-expand/scripts/` |
| Latinum quick-reference card | `skills/latinum-help/SKILL.md` |
| LatinumCrew decision guide (when to delegate to subagents) | `skills/latinumcrew/SKILL.md` |
| latinumcrew subagent definitions | `agents/latinumcrew-investigator.md`, `agents/latinumcrew-builder.md`, `agents/latinumcrew-reviewer.md` |
| Auto-activation rule body (Cursor/Windsurf/Cline/Copilot) | `src/rules/latinum-activate.md` |
| Add support for a new agent | `bin/install.js` (PROVIDERS array) |
| Per-repo init script (drops rule files into a user's repo) | `src/tools/latinum-init.js` |
| Claude Code hooks | `src/hooks/latinum-activate.js`, `src/hooks/latinum-mode-tracker.js`, `src/hooks/latinum-config.js`, `src/hooks/latinum-statusline.sh`, `src/hooks/latinum-statusline.ps1` |
| Settings.json read/write helpers | `bin/lib/settings.js` |
| MCP inflate server | `src/mcp-servers/latinum-inflate/` |
...
| `dist/latinum.skill` | ZIP of `skills/latinum/` (gitignored; rebuilt by CI on each push to `main`) |

`latinum-commit`, `latinum-review`, `latinum-help`, and `latinum-stats` are **not** mirrored under `plugins/latinum/skills/` by CI. Claude Code reaches them through the standalone hook + skill install path and `npx skills` carries them to other agents. If you see `plugins/latinum/skills/latinum-stats/` checked in, treat it as a legacy hand-committed copy — the workflow in `.github/workflows/sync-skill.yml` does not touch it.
...
4. If the skill should be in the Claude Code plugin, add a sync step to `.github/workflows/sync-skill.yml` so CI mirrors it into `plugins/latinum/skills/<name>/`.
...
# Expand-skill safety tests (Python)
python3 -m unittest tests.test_expand_safety

# Per-repo init tests
node tests/test_latinum_init.js
...
- **Conventional Commits** for the commit subject. See `skills/latinum-commit/SKILL.md` for the format we use here.
...
- **Mention the CI sync.** If you edited a source-of-truth file, note it: "CI will resync `plugins/latinum/skills/...` on merge."

PR descriptions don't need to be long. Latinum style fine. Just say what change, why.
...
- **Hooks must silent-fail on filesystem errors.** A `try/catch` that swallows the error is correct here. A hook that throws blocks Claude Code session start — that's user-facing breakage. See existing patterns in `src/hooks/latinum-activate.js`.
...
- **Symlink-safe flag writes via `safeWriteFlag()`** in `src/hooks/latinum-config.js`. The flag file lives at a predictable path under `$CLAUDE_CONFIG_DIR/`; without `O_NOFOLLOW` and a parent-symlink check, a local attacker can clobber any file the user can write.
...
Latinum like contribution. You bring bar, latinum put bar in pile. Pile
get bigger. Profit still big.
