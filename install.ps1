# latinum — installer shim (Windows / PowerShell).
#
...
if (-not $node) {
  Write-Error @"
latinum: Node.js (>=18) required. Install:
...
if ($nodeMajor -lt 18) {
  Write-Error "latinum: Node $nodeMajor too old. Need Node >=18. Upgrade: https://nodejs.org"
  exit 1
}
...
if (-not $npx) {
  Write-Error "latinum: npx required (ships with Node >=18). Reinstall Node.js."
  exit 1
}
