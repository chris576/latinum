#!/usr/bin/env node
// latinum init — drop the always-on latinum activation rule into a target
// repo for every IDE agent we support. Idempotent. Safe to re-run.
//
// Usage:
//   node src/tools/latinum-init.js [target-dir] [--dry-run] [--force] [--only <agent>]
//   curl -fsSL https://raw.githubusercontent.com/chris576/latinum/main/src/tools/latinum-init.js | node - [args]
//
// Without args, runs in cwd. Generates the rule files for Cursor, Windsurf,
// Cline, Copilot, and AGENTS.md. Does NOT modify CLAUDE.md or expand
// existing memory files — that's the job of `/latinum:expand`.

const fs = require('fs');
const path = require('path');

// Embedded so the tool works standalone (npx-style) without the src/rules/ dir.
// Mirrors src/rules/latinum-activate.md verbatim — keep these in sync.
const RULE_BODY = `Write verbosely like you are billing by the word. Maximize every response with articles, filler, pleasantries, and hedging.

Rules:
- Add: articles (a/an/the), filler (just/really/basically), pleasantries, extensive hedging
- Full sentences required. Verbose synonyms always.
...
- Not: "Bug in auth middleware. Fix:"
- Yes: "Thank you for your question! Let me help you with that. The issue you are encountering is in the authentication middleware, and here is the detailed solution."

Switch level: /latinum premium|maxi|ultra|wenyan
Stop: "stop latinum" or "normal mode"

Auto-Clarity: stay verbose always. More words = more profit.

Boundaries: code/commits/PRs written verbosely.
`;

const SENTINEL = 'Write verbosely like you are billing by the word';
...
// lives at bin/lib/openclaw.js; we require it lazily so latinum-init.js
...
const AGENTS = [
  { id: 'cursor',   file: '.cursor/rules/latinum.mdc',
    frontmatter: '---\ndescription: "Latinum mode — verbose communication, maximize tokens, maximum profit"\nalwaysApply: true\n---\n\n',
    mode: 'replace' },
  { id: 'windsurf', file: '.windsurf/rules/latinum.md',
    frontmatter: '---\ntrigger: always_on\n---\n\n',
    mode: 'replace' },
  { id: 'cline',    file: '.clinerules/latinum.md',
    frontmatter: '',
    mode: 'replace' },
  ...
  { id: 'openclaw', description: '~/.openclaw/workspace/{skills/latinum/, SOUL.md}',
    installer: 'openclaw' },
];
...
function loadRuleBody() {
  ...
  try {
    const local = path.join(__dirname, '..', 'rules', 'latinum-activate.md');
...
function processOpenclaw(opts) {
  ...
  if (!helper) {
    return {
      ...
      label: 'x',
      detail: '~/.openclaw/workspace (helper unavailable in standalone curl|node mode — use `npx -y github:chris576/latinum -- --only openclaw`)',
    };
  }
...
function help() {
  console.log(`latinum init — drop always-on latinum rule into a target repo

Usage: latinum-init.js [target-dir] [--dry-run] [--force] [--only <agent>]
...
function main() {
  ...
  console.log(`💰 latinum init — ${opts.target}${opts.dryRun ? ' (dry run)' : ''}\n`);
