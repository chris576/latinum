#!/usr/bin/env node
// latinum — shared configuration resolver
//
// Resolution order for default mode:
//   1. LATINUM_DEFAULT_MODE environment variable
...
//      - $XDG_CONFIG_HOME/latinum/config.json (any platform, if set)
//      - ~/.config/latinum/config.json (macOS / Linux fallback)
//      - %APPDATA%\latinum\config.json (Windows fallback)
//   4. 'maxi'
...
const VALID_MODES = [
  'off', 'premium', 'maxi', 'ultra',
  'wenyan-premium', 'wenyan', 'wenyan-maxi', 'wenyan-ultra',
  'commit', 'review', 'expand'
];

function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'latinum');
  }
  ...
  return path.join(os.homedir(), '.config', 'latinum');
}
...
function getDefaultMode() {
  ...
  const envMode = process.env.LATINUM_DEFAULT_MODE;
...
// flag path (~/.claude/.latinum-active) with a symlink to clobber other files.
//
...
//
// Set LATINUM_DEBUG=1 to emit stderr diagnostics when flag writes are refused.
//
// Silent-fails on any filesystem error — the flag is best-effort.
function safeWriteFlag(flagPath, content) {
  const debug = process.env.LATINUM_DEBUG === '1';
  try {
    ...
    try {
      ...
      if (lstat.isSymbolicLink()) {
        ...
        if (!realStat.isDirectory()) {
          if (debug) process.stderr.write(`[latinum] safeWriteFlag: symlink target ${realFlagDir} is not a directory\n`);
          return;
        }
        if (typeof process.getuid === 'function') {
          if (realStat.uid !== process.getuid()) {
            if (debug) process.stderr.write(`[latinum] safeWriteFlag: symlink target ${realFlagDir} owned by uid ${realStat.uid}, not current user ${process.getuid()}\n`);
            return;
          }
        } else {
          ...
          if (!normalizedReal.toLowerCase().startsWith(normalizedHome.toLowerCase() + path.sep) &&
            ...
            if (debug) process.stderr.write(`[latinum] safeWriteFlag: symlink target ${normalizedReal} is outside home directory ${normalizedHome}\n`);
            return;
          }
        }
      } else {
    ...
    const tempPath = path.join(realFlagDir, `.latinum-active.${process.pid}.${Date.now()}`);
...
// ($CLAUDE_CONFIG_DIR/.latinum-history.jsonl).
//
  ...
  try {
    ...
    try {
      ...
      if (lstat.isSymbolicLink()) {
        ...
        if (typeof process.getuid === 'function') {
          if (realStat.uid !== process.getuid()) {
            if (debug) process.stderr.write(`[latinum] appendFlag: symlink target ${realDir} owned by uid ${realStat.uid}\n`);
            return;
          }
        } else {
          const home = os.homedir();
