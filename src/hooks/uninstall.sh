#!/bin/bash
# latinum — uninstaller for the SessionStart + UserPromptSubmit hooks
# Removes: hook files in ~/.claude/hooks, settings.json entries, and the flag file
# Usage: bash src/hooks/uninstall.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/chris576/latinum/main/src/hooks/uninstall.sh)
set -e

CLAUDE_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}"
HOOKS_DIR="$CLAUDE_DIR/hooks"
SETTINGS="$CLAUDE_DIR/settings.json"
FLAG_FILE="$CLAUDE_DIR/.latinum-active"
...
# Detect if latinum is installed as a plugin (check plugin cache)
PLUGIN_INSTALLED=0
if [ -d "$CLAUDE_DIR/plugins" ]; then
  if find "$CLAUDE_DIR/plugins" -path "*/latinum*" -name "plugin.json" -print -quit 2>/dev/null | grep -q .; then
    PLUGIN_INSTALLED=1
  fi
fi

if [ "$PLUGIN_INSTALLED" -eq 1 ]; then
  echo "Latinum appears to be installed as a Claude Code plugin."
  echo "To uninstall the plugin, run:"
  echo ""
  echo "  claude plugin disable latinum"
  echo ""
...
echo "Uninstalling latinum hooks..."
...
if [ -f "$SETTINGS" ]; then
  ...
  if ! command -v node >/dev/null 2>&1; then
    ...
    echo "         Remove the latinum SessionStart and UserPromptSubmit"
    ...
    LATINUM_SETTINGS="$SETTINGS" LATINUM_HOOKS_DIR="$HOOKS_DIR" node -e "
      ...
      // Remove statusLine if it references latinum
...
echo ""
echo "Other agents:"
echo "  npx skills remove latinum    # Cursor, Windsurf, Cline, Copilot, etc."
echo "  claude plugin disable latinum  # Claude Code plugin"
echo "  gemini extensions uninstall latinum  # Gemini CLI"
