# latinum — one-command hook installer for Claude Code (Windows PowerShell)
...
#        irm https://raw.githubusercontent.com/chris576/latinum/main/src/hooks/install.ps1 | iex
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    Write-Host "ERROR: 'node' is required to install the latinum hooks (used to merge" -ForegroundColor Red
...
$RepoUrl = "https://raw.githubusercontent.com/chris576/latinum/main/hooks"
...
if (-not $Force) {
    ...
    if ($AllFilesPresent -and (Test-Path $Settings)) {
        try {
                ...
                foreach ($entry in $entries) {
                    if ($entry.hooks) {
                        foreach ($hookDef in $entry.hooks) {
                            if ($hookDef.command -and $hookDef.command.Contains("latinum")) {
                                return $true
                            }
                        }
                    }
            ...
            $HooksWired = (& $hasLatinumHook "SessionStart") -and (& $hasLatinumHook "UserPromptSubmit")
    ...
    if ($AllFilesPresent -and $HooksWired -and $HasStatusLine) {
        Write-Host "Latinum hooks already installed in $HooksDir"
...
if ($Force -and (Test-Path (Join-Path $HooksDir "latinum-activate.js"))) {
    Write-Host "Reinstalling latinum hooks (-Force)..."
} else {
    Write-Host "Installing latinum hooks..."
}
...
$tmpScript = Join-Path $env:TEMP "latinum-install-$([System.Diagnostics.Process]::GetCurrentProcess().Id).js"
try {
...
Write-Host "  - SessionStart hook: auto-loads latinum rules every session"
Write-Host "  - Mode tracker hook: updates statusline badge when you switch modes"
Write-Host "    (/latinum lite, /latinum ultra, /latinum-commit, etc.)"
Write-Host "  - Statusline badge: shows [LATINUM] or [LATINUM:ULTRA] etc."
