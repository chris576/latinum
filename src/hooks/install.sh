#!/bin/bash
# latinum — one-command hook installer for Claude Code
# Installs: SessionStart hook (auto-load rules) + UserPromptSubmit hook (mode tracking)
# Usage: bash src/hooks/install.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/chris576/latinum/main/src/hooks/install.sh)
...
if ! command -v node >/dev/null 2>&1; then
  echo "ERROR: 'node' is required to install the latinum hooks (used to merge"
...
REPO_URL="https://raw.githubusercontent.com/chris576/latinum/main/hooks"

HOOK_FILES=("package.json" "latinum-config.js" "latinum-activate.js" "latinum-mode-tracker.js" "latinum-stats.js" "latinum-statusline.sh")
...
if [ "$FORCE" -eq 0 ]; then
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ -f "$SETTINGS" ]; then
    if LATINUM_SETTINGS="$SETTINGS" node -e "
      const fs = require('fs');
      const settings = JSON.parse(fs.readFileSync(process.env.LATINUM_SETTINGS, 'utf8'));
      const hasLatinumHook = (event) =>
        Array.isArray(settings.hooks?.[event]) &&
        settings.hooks[event].some(e =>
          e.hooks && e.hooks.some(h => h.command && h.command.includes('latinum'))
        );
      process.exit(
        hasLatinumHook('SessionStart') &&
        hasLatinumHook('UserPromptSubmit') &&
        !!settings.statusLine
          ? 0
          : 1
      );
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ "$HOOKS_WIRED" -eq 1 ] && [ "$HAS_STATUSLINE" -eq 1 ]; then
    ALREADY_INSTALLED=1
    echo "Latinum hooks already installed in $HOOKS_DIR"
...
if [ "$FORCE" -eq 1 ] && [ -f "$HOOKS_DIR/latinum-activate.js" ]; then
  echo "Reinstalling latinum hooks (--force)..."
else
  echo "Installing latinum hooks..."
fi
...
chmod +x "$HOOKS_DIR/latinum-statusline.sh"
...
LATINUM_SETTINGS="$SETTINGS" LATINUM_HOOKS_DIR="$HOOKS_DIR" node -e "
  const fs = require('fs');
  const settingsPath = process.env.LATINUM_SETTINGS;
  const hooksDir = process.env.LATINUM_HOOKS_DIR;
  const managedStatusLinePath = hooksDir + '/latinum-statusline.sh';
  const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
  if (!settings.hooks) settings.hooks = {};

  // SessionStart — auto-load latinum rules
  ...
  if (!hasStart) {
    settings.hooks.SessionStart.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/latinum-activate.js\"',
        timeout: 5,
  ...
  // UserPromptSubmit — track mode changes when user types /latinum commands
  ...
  if (!hasPrompt) {
    settings.hooks.UserPromptSubmit.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/latinum-mode-tracker.js\"',
        timeout: 5,
  ...
  // Statusline — wire latinum badge (report if skipped)
  if (!settings.statusLine) {
    ...
    if (cmd.includes(managedStatusLinePath)) {
      ...
      console.log('  NOTE: Existing statusline detected — latinum badge NOT added.');
...
echo "  - SessionStart hook: auto-loads latinum rules every session"
echo "  - Mode tracker hook: updates statusline badge when you switch modes"
echo "    (/latinum lite, /latinum ultra, /latinum-commit, etc.)"
echo "  - Statusline badge: shows [LATINUM] or [LATINUM:ULTRA] etc."
