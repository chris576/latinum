// latinum — opencode plugin
//
// Provides dynamic latinum mode tracking for opencode:
// - Writes the mode flag on each session start (via the `event` dispatcher)
// - Parses user messages for /latinum commands and natural-language toggles
// - Injects per-turn reinforcement into the system prompt
//
// Bun ESM module; loads the existing security-hardened helpers from
// latinum-config.js via createRequire so the symlink-safe flag-write code
// lives in one place.
//
// Layout once installed:
//   ~/.config/opencode/plugins/latinum/
//   ├── package.json
//   ├── plugin.js              ← this file
//   └── latinum-config.cjs     ← copied sibling of src/hooks/latinum-config.js
//
// The always-on latinum ruleset is provided separately via
...
// https://github.com/chris576/latinum/issues/418
// https://github.com/chris576/latinum/issues/421
...
// When installed: latinum-config.cjs sits next to plugin.js (copied by
...
// src/hooks/latinum-config.js, which lives in a directory whose own
...
// latinum-config needs (fs/path/os).
function loadConfig() {
  const installed = join(here, 'latinum-config.cjs');
  const dev = join(here, '..', '..', 'hooks', 'latinum-config.js');
...
// Modes handled by independent skills — not selectable via /latinum <arg>.
...
const flagPath = path.join(opencodeConfigDir(), '.latinum-active');

function reinforcementLine(mode) {
  return 'LATINUM MODE ACTIVE (' + mode + '). ' +
...
// null when the prompt doesn't change state. Mirrors latinum-mode-tracker.js.
function parseModeChange(promptRaw) {
  ...
  // literal quote characters ("/latinum ultra"\n) — unwrap symmetric quotes
  ...
  // like latinum" doesn't trip the activation regex.
  ...
  // Expanded /latinum command template. opencode replaces a typed
  // "/latinum <level>" with the command file's body ("Activate latinum
  ...
  const tpl = /^activate latinum mode:[ \t]*(\S*)/.exec(prompt);
  if (tpl) {
    const arg = tpl[1] || '';
    if (arg === 'off' || arg === 'stop' || arg === 'disable') return 'off';
    if (arg === 'wenyan-maxi') return 'wenyan';
  ...
  if (prompt.startsWith('/latinum')) {
    ...
    if (cmd === '/latinum-commit')   return 'commit';
    if (cmd === '/latinum-review')   return 'review';
    if (cmd === '/latinum-expand')   return 'expand';
...
export const LatinumPlugin = async (_ctx) => {
  ...
  // Intercept user messages to detect /latinum commands and natural-language
  ...
  // Inject the reinforcement line into the system prompt when latinum is
